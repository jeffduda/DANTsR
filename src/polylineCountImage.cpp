...
SEXP polylineCountImage( SEXP r_mesh, SEXP r_image )
{
  ...
  using FilterType = itk::PolyLineMeshToCellCountImageFilter<MeshType, OutputImageType>;
...
try
{
  //Rcpp::Rcout << "polylineCountImage()" << std::endl;
  ...
  if( pixeltype == "double" )
    ...
    if ( dimension == 2 )
      ...
      return  polylineCountImage< MeshType, ImageType >( r_mesh, r_image );
      }
    else if ( dimension == 3 )
      {
